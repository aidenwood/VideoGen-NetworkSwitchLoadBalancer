...
share and gives every teammate **live tray status + a notification sound** each time a
job is dispatched, picked up, or finished — so you know the farm's working without
watching a terminal. Build it with `cd desktop && npm install && npm run tauri build`,
then drop `LTX Mac Farm.app` in `/Applications` on each Mac. Details in
