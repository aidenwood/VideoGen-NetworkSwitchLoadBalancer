...
fn open_script_in_terminal(cfg: &Config, name: &str) -> Result<String, String> {
    let dir = detect_repo(cfg).ok_or_else(|| {
        format!(
            "Can't find the farm scripts (looking for farm_worker.sh under {}/Desktop, \
             Documents, Downloads and Developer). Use “Choose folder…” to point at the \
             folder you cloned.",
            home()
        )
    })?;
    ...
    // `open -a Terminal <script>` starts a fresh login shell and passes NO
    // environment, so the script would fall back to its own default of
    // FARM_ROOT=/Volumes/RenderFarm. On a coordinator that's the wrong folder
    // and provision.command aborts with "share not mounted". Wrap it in a
    // throwaway .command that exports this Mac's real config first.
    let lora_dir = if cfg.lora_dir.trim().is_empty() {
        format!("{}/farm-loras", home())
    } else {
        cfg.lora_dir.trim().to_string()
    };
    let wrapper = std::env::temp_dir().join(format!("ltx-farm-{}", name));
    let body = format!(
        "#!/bin/bash\n\
         # generated by LTX Mac Farm — passes the app's config into {name}\n\
         export FARM_ROOT={root}\n\
         export LTX_DIR={ltx}\n\
         export LORA_DIR={lora}\n\
         export COORDINATOR={coord}\n\
         cd {dir} || exit 1\n\
         exec ./{name}\n",
        name = name,
    ...
    std::fs::write(&wrapper, body).map_err(|e| e.to_string())?;
    #[cfg(unix)]
    {
